...
// Borrowing generation logic from https://github.com/coredns/coredns/blob/master/directives_generate.go
// Overriding defaults since the 'in-tree' plugins are not part of this repo
...
func genImports(file, pack string, mi map[string]string) {
	...
	for _, v := range mi {
		if strings.HasPrefix(v, githubOrg) {
			...
			thirdPartyImports += `_ "` + pluginPath + v + `"` + "\n"
		}
	}
	outs += coreDnsImports
